...
float shift_addition_cc2(complexf *input, complexf* output, int input_size, shift_addition_data_t d, float starting_phase)
{
	...
	float32x4_t cssc, iqiq, cscsdelta;
	cscsdelta = vsetq_lane_f32(d.cosdelta, cscsdelta, 0);
	cscsdelta = vsetq_lane_f32(d.sindelta, cscsdelta, 1);
	cscsdelta = vsetq_lane_f32(d.cosdelta, cscsdelta, 2);
	cscsdelta = vsetq_lane_f32(d.sindelta, cscsdelta, 3);

	for(int i=0;i<input_size;i++) //@shift_addition_cc: work
	{
		cssc = vsetq_lane_f32(cosphi, cssc, 0);
		cssc = vsetq_lane_f32(cosphi, cssc, 3);
		cssc = vsetq_lane_f32(sinphi, cssc, 1);
		cssc = vsetq_lane_f32(sinphi, cssc, 2);

		// load I/Q sample
		float32x2_t sample;
		sample = vld1_f32((float *)(input+i));
		iqiq = vcombine_f32(sample,sample);

		iqiq = vmulq_f32(iqiq, cssc);
		// to be optimized...
		iof(output,i)=vgetq_lane_f32(iqiq,0)-vgetq_lane_f32(iqiq,1);
		qof(output,i)=vgetq_lane_f32(iqiq,2)+vgetq_lane_f32(iqiq,3);
		...
		cssc = vmulq_f32(cssc, cscsdelta);

		cosphi = vgetq_lane_f32(cssc,0)-vgetq_lane_f32(cssc,1);
		sinphi = vgetq_lane_f32(cssc,2)+vgetq_lane_f32(cssc,3);	
		//cosphi_last=cosphi;
		//sinphi_last=sinphi;
